...
/*
 * UTF-8 decode inspired from rosetta code https://rosettacode.org/wiki/UTF-8_encode_and_decode#C
 */
utf_t * utf[] = {
...
*/
void drawChar(GFXfont* font, uint8_t* buffer, int* cursor_x, uint16_t buf_width, uint16_t buf_height, int16_t baseline_height, uint32_t cp) {
 ...
 */
void getCharBounds(GFXfont* font, uint32_t cp, int* x, int* y, int* minx, int* miny, int* maxx, int* maxy) {
    GFXglyph *glyph;
    get_glyph(font, cp, &glyph);

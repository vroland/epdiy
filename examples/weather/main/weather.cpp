...
#ifdef BUILTIN_LED
    pinMode(BUILTIN_LED, INPUT);  // If it's On, turn it off and some boards use GPIO-5 for SPI-SS,
                                  // which remains low after screen use
...
boolean SetupTime() {
    configTime(
        gmtOffset_sec, daylightOffset_sec, ntpServer, "time.nist.gov"
...
bool DecodeWeather(WiFiClient& json, String Type) {
    ...
    if (Type == "weather") {
        // All Serial.println statements are for diagnostic purposes and not required, remove if not
        // needed
...
void DisplayWeather() {                          // 9.7" e-paper display is 1200x825 resolution
    ...
    DisplayMainWeatherSection(137, 130);  // Centre section of display for Location, temperature,
                                          // Weather report, current Wx Symbol and wind direction
...
void DisplayMainWeatherSection(int x, int y) {  // (x=500, y=190)
    ...
    DisplayPressureSection(
        x + 160, y + 70, 180, 170, WxConditions[0].Pressure, WxConditions[0].Trend
    );
...
void DisplayDisplayWindSection(int x, int y, float angle, float windspeed, int Cradius) {
    ...
    for (float a = 0; a < 360; a = a + 22.5) {
        ...
        if (a == 45)
            drawString(dxo + x + 27, dyo + y - 20, TXT_NE, CENTER);
        if (a == 135)
            drawString(dxo + x + 27, dyo + y - 2, TXT_SE, CENTER);
        if (a == 225)
            drawString(dxo + x - 43, dyo + y - 2, TXT_SW, CENTER);
        if (a == 315)
            drawString(dxo + x - 43, dyo + y - 20, TXT_NW, CENTER);
        dxi = dxo * 0.9;
        dyi = dyo * 0.9;
...
void DisplayTemperatureSection(int x, int y, int twidth, int tdepth) {
    ...
    setFont(OpenSans24B);
    drawString(
        x, y, String(WxConditions[0].Temperature, 1) + "°C", CENTER
...
void DisplayForecastTextSection(int x, int y, int fwidth, int fdepth) {
    ...
    if (Wx_Description.indexOf("(") > 0)
        Wx_Description += ")";
    int MsgWidth = 43;  // Using proportional fonts, so be aware of making it too wide!
    if (Language == "DE")
        drawStringMaxWidth(
            x + 30, y + 40, MsgWidth, Wx_Description, LEFT
        );  // Leave German text in original format, 28 character screen width at this font size
    else
        drawStringMaxWidth(
            x + 30, y + 40, MsgWidth, TitleCase(Wx_Description), LEFT
        );  // 28 character screen width at this font size
}
...
void DisplayPrecipitationSection(int x, int y, int pwidth, int pdepth) {
    ...
    if (WxForecast[1].Rainfall >= 0.005) {  // Ignore small amounts
        drawString(
            x, y + 80, String(WxForecast[1].Rainfall, 2) + (Units == "M" ? "mm" : "in"), LEFT
...
void DisplayForecastSection(int x, int y) {
    ...
    // drawString(SCREEN_WIDTH / 2 - 50, gy - 50, TXT_FORECAST_VALUES, CENTER); // Based on a graph
    // height of 60 setFont(OpenSans12);
...
boolean UpdateLocalTime() {
    ...
    if (Units == "M") {
        ...
        strftime(
            update_time, sizeof(update_time), "%H:%M:%S", &timeinfo
        );  // Creates: '@ 14:05:49'   and change from 30 to 8
            // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        sprintf(time_output, "%s", update_time);
    } else {
        strftime(
            day_output, sizeof(day_output), "%a %b-%d-%Y", &timeinfo
...
void addcloud(int x, int y, int scale, int linesize) {
    ...
    fillCircle(
        x + scale * 1.5, y - scale * 1.3, scale * 1.75, GxEPD_BLACK
    );  // Right middle upper circle
    fillRect(
        x - scale * 3 - 1, y - scale, scale * 6, scale * 2 + 1, GxEPD_BLACK
    ...
    fillCircle(
        x - scale, y - scale, scale * 1.4 - linesize, GxEPD_WHITE
...
void addrain(int x, int y, int scale, bool IconSize) {
    ...
    for (int d = 0; d < 4; d++) {
        addraindrop(
            x + scale * (7.8 - d * 1.95) - scale * 5.2, y + scale * 2.1 - scale / 6, scale / 1.6
        );
    }
}
...
void addsun(int x, int y, int scale, bool IconSize) {
    ...
    if (IconSize == LargeIcon) {
        drawLine(
            1 + x - scale * 1.3, y - scale * 1.3, 1 + x + scale * 1.3, y + scale * 1.3, GxEPD_BLACK
        );
        drawLine(
            2 + x - scale * 1.3, y - scale * 1.3, 2 + x + scale * 1.3, y + scale * 1.3, GxEPD_BLACK
        );
        drawLine(
            3 + x - scale * 1.3, y - scale * 1.3, 3 + x + scale * 1.3, y + scale * 1.3, GxEPD_BLACK
        );
        drawLine(
            1 + x - scale * 1.3, y + scale * 1.3, 1 + x + scale * 1.3, y - scale * 1.3, GxEPD_BLACK
        );
...
/* (C) D L BIRD
    This function will draw a graph on a ePaper/TFT/LCD display using data from an array containing
   data to be graphed. The variable 'max_readings' determines the maximum number of data elements
   for each array. Call it with the following parametric data: x_pos-the x axis top-left position of
   the graph y_pos-the y-axis top-left position of the graph, e.g. 100, 200 would draw the graph 100
   pixels along and 200 pixels down from the top-left of the screen width-the width of the graph in
   pixels height-height of the graph in pixels Y1_Max-sets the scale of plotted data, for example
   5000 would scale all data to a Y-axis of 5000 maximum data_array1 is parsed by value, externally
   they can be called anything else, e.g. within the routine it is called data_array1, but
   externally could be temperature_readings auto_scale-a logical value (TRUE or FALSE) that switches
   the Y-axis autoscale On or Off barchart_on-a logical value (TRUE or FALSE) that switches the
   drawing mode between barhcart and line graph barchart_colour-a sets the title and graph plotting
   colour If called with Y!_Max value of 500 and the data never goes above 500, then autoscale will
   retain a 0-500 Y scale, if on, the scale increases/decreases to match the data.
...
void ReportEvent(String EventMessage[]) {
    ...
    for (byte Event = 1; Event <= EventCnt; Event++) {
        ...
        Serial.println(
            "Evnt#" + String(Event < 10 ? "0" : "") + String(Event) + " : " + EventMessage[Event]
        );
    }
    epd_poweroff();
}

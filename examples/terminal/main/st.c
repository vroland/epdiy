...
enum cursor_state { CURSOR_DEFAULT = 0, CURSOR_WRAPNEXT = 1, CURSOR_ORIGIN = 2, CURSOR_HIDE = 4 };
...
void* xrealloc_32(void* p, size_t len) {
...
char base64dec_getc(const signed char** src) {
...
void tresize(int col, int row) {
    ...
    if (line_render_queue == NULL) {
        ...
        RTOS_ERROR_CHECK(xTaskCreatePinnedToCore(&render_line, "lr1", 1 << 12, NULL, 5, NULL, 1));
        RTOS_ERROR_CHECK(xTaskCreatePinnedToCore(&render_line, "lr2", 1 << 12, NULL, 5, NULL, 1));
    }
...
static uint8_t displaycolor(uint32_t col) {
    if (IS_TRUECOL(col)) {
        return (30 * ((col & 0xFF0000) >> 16) + 59 * ((col & 0x00FF00) >> 8) + 11 * (col & 0x0000FF)
               ) /
               100 / 16;
    }
...
static void render_line() {
    while (true) {
        ...
        while (idx < term.col) {
            int data_pos = 0;
            Glyph c = glyphs[idx];
            while (idx < term.col && (glyphs[idx].fg == c.fg && glyphs[idx].bg == c.bg &&
        ...
        if (height > 0) {
            ...
            mask_buffer(
                mask_start, (uint32_t*)start_ptr, (uint32_t*)(render_masked_buf + y_offset), height
            );
        }
...
static void full_refresh() {
    ...
    epd_poweron();
    epd_draw_base(
        epd_full_screen(), render_fb_front, epd_full_screen(),
        MODE_PACKING_2PPB | MODE_EPDIY_BLACK_TO_GL16 | PREVIOUSLY_BLACK, temperature, NULL,
...
void epd_render(void) {
    ...
    if (drawn_lines) {
        ...
        if (height > 0) {
            ...
            enum EpdDrawError err = epd_draw_base(
                epd_full_screen(), render_masked_buf, epd_full_screen(),
                MODE_PACKING_2PPB | MODE_EPDIY_BLACK_TO_GL16 | PREVIOUSLY_BLACK, temperature,
                boolean_line_dirtyness, EPD_BUILTIN_WAVEFORM
            );
            epd_poweroff();

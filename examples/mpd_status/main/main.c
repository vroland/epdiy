...
void show_status(struct mpd_status* status, struct mpd_song* song, int album_height) {
    ...
    if (mpd_song_get_tag(song, MPD_TAG_TITLE, 0)) {
        epd_write_default(
            &FiraSans24, mpd_song_get_tag(song, MPD_TAG_TITLE, 0), &cursor_x, &cursor_y,
    ...
    if (mpd_song_get_tag(song, MPD_TAG_ALBUM, 0)) {
        ...
        epd_write_default(
            &FiraSans20, mpd_song_get_tag(song, MPD_TAG_ALBUM, 0), &cursor_x, &cursor_y,
            epd_hl_get_framebuffer(&hl)
        );
    }
    ...
    if (mpd_song_get_tag(song, MPD_TAG_ARTIST, 0)) {
        cursor_x = album_cover_x;
        cursor_y = album_height + 100 + FiraSans24.advance_y + FiraSans20.advance_y * 2;
        epd_write_default(
            &FiraSans20, mpd_song_get_tag(song, MPD_TAG_ARTIST, 0), &cursor_x, &cursor_y,
            epd_hl_get_framebuffer(&hl)
        );
    }
}
...
void epd_task() {
    ...
    while (true) {
        ...
        if ((do_update || init) && new_info != NULL) {
                ...
                switch (mpd_entity_get_type(entity)) {
                    ...
                    case MPD_ENTITY_TYPE_SONG:
                        ...
                        if (strcmp(
                                ...
                                mpd_song_get_uri(playback_info->current_song)
                            ) == 0) {
                        ...
                        epd_write_string(
                            font, timestr, &timestamp_x, &timestamp_y, img_buf, &rightalign
                        );
                        break;
            ...
            epd_draw_line(
                queue_x_start, queue_y - FiraSans16.advance_y / 2, queue_x_end,
                queue_y + 1 - FiraSans16.advance_y / 2, 0, img_buf
            );
            ...
            if (album_cover == NULL || album_cover->identifier == NULL || album == NULL ||
                strncmp(album_cover->identifier, album, 128) != 0) {
                if (album_cover) {
                    free_album_cover(album_cover);
                    album_cover = NULL;
                }
                album_cover = readpicture(
                    mpd_conn, (char*)mpd_song_get_uri(playback_info->current_song), album
                );
            }
            ...
            if (album_cover != NULL) {
                ...
                printf(
                    "album cover dimensions: %" PRIu32 "x%" PRIu32 "\n", album_cover->width,
                    album_cover->height
                );

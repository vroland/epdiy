...
//====================================================================================
int decodeJpeg(uint8_t* source_buf, int img_bytes_len, int xpos, int ypos) {
    uint32_t decode_start = esp_timer_get_time();

    if (strcmp(DISPLAY_COLOR_TYPE, (char*)"NONE") == 0) {
        if (jpeg.openRAM(source_buf, img_bytes_len, JPEGDraw4Bits)) {
        ...
        if (jpeg.openRAM(source_buf, img_bytes_len, JPEGDrawRGB)) {
...
void read_file(char * filename) {
    ...
    if (file == NULL) {
        ...
        fread(source_buf, 1, file_size, file);
        fclose(file);

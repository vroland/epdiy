...
static void epd_board_init(uint32_t epd_row_width, const EpdInitConfig* init_config) {
    ...
    ESP_ERROR_CHECK(
        epd_board_i2c_init(&config_reg.i2c, &board_i2c_config, init_config, true, true)
    );
...
static void epd_board_set_ctrl(epd_ctrl_state_t* state, const epd_ctrl_state_t* const mask) {
    ...
    if (mask->ep_output_enable || mask->ep_mode || mask->ep_stv) {
        ...
        if (err != ESP_OK) {
            ...
            // retry once, abort if it fails again
...
static void epd_board_poweron(epd_ctrl_state_t* state) {
    ...
    while (retry_count < 30) {
        ...
        if (pca9555_read_input(config_reg.i2c.pca, 1) & CFG_PIN_PWRGOOD) {

...
static void IRAM_ATTR calc_epd_input_1bpp(const uint32_t *line_data,
  ...
  for (uint32_t j = 0; j < epd_display.width / 16; j++) {
    uint8_t v1 = *(data_ptr++);
    uint8_t v2 = *(data_ptr++);
...
static void IRAM_ATTR calc_epd_input_1ppB(const uint32_t *ld,
  ...
  // this is reversed for little-endian, but this is later compensated
  // through the output peripheral.
  for (uint32_t j = 0; j < epd_display.width / 4; j += 4) {
...
void IRAM_ATTR provide_out(OutputParams *params) {
  while (true) {
    ...
    memset(line, 255, epd_display.width);
    ...
    for (int i = 0; i < epd_display.height; i++) {
      ...
      bool shifted = false;
      if (area.width == epd_display.width && area.x == 0 && !crop && !params->error) {
        lp = (uint32_t *)ptr;
        ptr += bytes_per_line;
        ...
        line_bytes = min(line_bytes, epd_display.width / width_divider -
        ...
        if (ppB == 2) {
          ...
          if (cropped_width % 2 == 1 &&
              min_x / 2 + cropped_width / 2 + 1 < epd_display.width) {
            *(buf_start + line_bytes - 1) |= 0xF0;
          }
          if (area.x % 2 == 1 && !(crop_x % 2 == 1) && min_x < epd_display.width) {
            shifted = true;
            uint32_t remaining =
                (uint32_t)line + epd_display.width / 2 - (uint32_t)buf_start;
          ...
          if (cropped_width % 8 != 0 && bytes_per_line + 1 < epd_display.width) {
            uint8_t mask = 0;
          ...
          if (min_x % 8 != 0 && min_x < epd_display.width) {
            // shift to right
            shifted = true;
            uint32_t remaining =
                (uint32_t)line + epd_display.width / 8 - (uint32_t)buf_start;
            uint32_t to_shift = min(line_bytes + 1, remaining);
      ...
      if (shifted) {
        memset(line, 255, epd_display.width / width_divider);
      }
    }
...
void IRAM_ATTR feed_display(OutputParams *params) {
  while (true) {
    ...
    line_start_x = min(max(line_start_x, 0), epd_display.width);
    line_end_x = min(max(line_end_x, 0), epd_display.width);
    ...
    for (int i = 0; i < epd_display.height; i++) {
      ...
      if (!params->error) {
        ...
        if (line_start_x > 0 || line_end_x < epd_display.width) {

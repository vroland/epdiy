...
EpdRect epd_full_screen() {
  EpdRect area = {.x = 0, .y = 0, .width = epdiy_display.width, .height = epdiy_display.height};
  return area;
}
...
Coord_xy _rotate(uint16_t x, uint16_t y) {
    switch (display_rotation) {
        ...
        case EPD_ROT_PORTRAIT:
            _swap_int(x, y);
            x = epdiy_display.width - x - 1;
        break;
        case EPD_ROT_INVERTED_LANDSCAPE:
            x = epdiy_display.width - x - 1;
            y = epdiy_display.height - y - 1;
        break;
...
void epd_draw_pixel(int x, int y, uint8_t color, uint8_t *framebuffer) {
  ...
  if (x < 0 || x >= epdiy_display.width) {
    return;
  }
  if (y < 0 || y >= epdiy_display.height) {
    return;
  }

  uint8_t *buf_ptr = &framebuffer[y * epdiy_display.width / 2 + x / 2];
  if (x % 2) {
...
void epd_copy_to_framebuffer(EpdRect image_area, const uint8_t *image_data,
  ...
  for (uint32_t i = 0; i < image_area.width * image_area.height; i++) {
    ...
    if (xx < 0 || xx >= epdiy_display.width) {
      continue;
    }
    int yy = image_area.y + i / image_area.width;
    if (yy < 0 || yy >= epdiy_display.height) {
      continue;
    }
    uint8_t *buf_ptr = &framebuffer[yy * epdiy_display.width / 2 + xx / 2];
    if (xx % 2) {
...
int epd_rotated_display_width() {
  int display_width = epdiy_display.width;
  switch (display_rotation) {
          case EPD_ROT_PORTRAIT:
              display_width = epdiy_display.height;
          break;
          case EPD_ROT_INVERTED_PORTRAIT:
              display_width = epdiy_display.height;
          break;
...
int epd_rotated_display_height() {
  int display_height = epdiy_display.height;
  switch (display_rotation) {
          case EPD_ROT_PORTRAIT:
              display_height = epdiy_display.width;
          break;
          case EPD_ROT_INVERTED_PORTRAIT:
              display_height = epdiy_display.width;
          break;
